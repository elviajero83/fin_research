...
def run(mini_batch):
    ...
    for raw_file_name in mini_batch:
        ...
        try:
            ...
            print("error in grouping data ")
            print(e)
            resultList.append("{}, {}, {}".format(os.path.basename(raw_file_name), df_1day_raw.shape, 'STR Error'))
            return resultList
...
if __name__ == "__main__":
    init()
    path_on_datastore = "datasets/quotes_raw/corrected"
    # path_on_datastore = "datasets/quotes_raw_sample"
    quotes_raw_dataset = Dataset.File.from_files(
        (
            default_datastore,
            path_on_datastore,
        ),
    ...
    quotes_raw_dataset.start()
    files_list = os.listdir(args.data_output)
    # files_list = [x for x in files_list if '.csv.gz' in x]
    files_list = [x.replace('-mt.csv','.csv.gz').replace('-mts.csv','.csv.gz') for x in files_list]
    print("there are {} files in the output dataset".format(len(files_list)))
    trades_merged_files = os.listdir('../data/trades_merged_mp/')
    print("there are {} files in the trades merged dataset".format(len(trades_merged_files)))
    trades_merged_files = [x.replace('-mt.csv','.csv.gz').replace('-mts.csv','.csv.gz') for x in trades_merged_files]
    remaining_files= set(trades_merged_files).difference(set(files_list))
    remaining_files = list(remaining_files)
    print("there are {} files remaining in the input dataset".format(len(remaining_files)))
    # pool = mp.Pool(mp.cpu_count())
    # results = pool.map(run, [[os.path.join(quotes_raw_dataset.mount_point,name)] for name in remaining_files])
    with open("quotes_merge_logs.log",'w') as f:
        for i in tqdm(range(len(remaining_files))):
            results = run([os.path.join(quotes_raw_dataset.mount_point,remaining_files[i])])
        f.writelines([str(x)+'\n' for x in results])
    quotes_raw_dataset.stop()